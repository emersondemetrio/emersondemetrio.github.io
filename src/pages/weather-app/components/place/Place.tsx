...
const Place = ({ name, timeZone, city, current }: PlaceProps) => {
  ...
  return (
    <div className={`h-40 w-full border-2 flex ${flexDir} ${justify} p-1 my-4 rounded-lg items-center bg-slate-600 shadow-lg ${current ? "border-amber-50" : "border-transparent"}`}>
      <div className={merge("ml-2", itemWidth, spacing)}>
        <h3>{name} {isLoading ? 'Loading...' : ''}</h3>
      </div>
      <div className={merge(itemWidth, spacing, "flex-col flex justify-between")}>
        <Clock timeZone={timeZone} />
        <span>{dateDiffInHours(timeZone, new Date())}{current && ", Current"}</span>
      </div>
      {!!error && (
        <div className="info">
      ...
      {showWeather && (
        <div className={merge(itemWidth, spacing)}>
          ...
          <div className="flex flex-row items-center">
            <div>
              {
                weather?.current?.condition?.icon && <img
            ...
            <div className="left-40 ml-5">
              <button className="btn btn-circle btn-info" disabled={isLoading} onClick={() => refetch(city)}>
                <Reload />
              </button>
              <button className="btn btn-circle btn-info" onClick={() => openNews(getNewsURL(city))}>
                <Info />
              </button>
            </div>
          </div>
